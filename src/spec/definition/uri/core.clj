(ns spec.definition.uri.core
  (:require
   ...
   [spec.definition.network.core :as sd-network]
     ...
     (gen/vector
       (gen/frequency
         [[1 (gen/fmap uri-norm/percent-encode
               (sd-string/gen-character-string-unicode
          ...
          [10 (sd-string/gen-character-string-unicode alpha-digit-pattern)]])
...
(defn gen-query-map
  ...
  ([{:keys [min-query-param-count
   ...
   (let [key-gen
         (sd-string/gen-string-unicode "[a-z]"
   ...
   (let [scheme-gen
         ...
         (gen/frequency
           [[(if (true? include-ipv4-addresses?) 1 0)
             (sd-network/gen-ipv4-address-string)]
            [9 (sd-domain/gen-domain-name)]])
         port-gen
         (gen/frequency
...
(sd-def/def-validate-pred http-uri-string?
