...
(deftest email-address-spec-validation
  (dt-test-cases/assert-cases-satisfied-by
    (partial spec/valid? :datatype.email/email-address)
...
(deftest email-address-spec-generation
  (testing "does not generate nil"
    (is (every? false?
          (map nil?
            (gen/sample
              (spec/gen :datatype.email/email-address) 100)))))
  ...
  (testing "generates unique email addresses"
    (let [email-addresses
          (into #{}
            (gen/sample
              (spec/gen :datatype.email/email-address)
              100))]
  ...
  (testing "generates email addresses with a variety of local parts"
    (let [email-addresses
          (gen/sample (spec/gen :datatype.email/email-address) 100)
          local-parts
          (into #{}
            (map
              (fn [email-address]
...
(deftest email-address-pred-requirement
  (is (= :must-be-an-email-address
        (sv-core/pred-requirement
          'datatype.email.core/email-address?))))
