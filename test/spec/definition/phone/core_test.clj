...
(deftest phone-number-string-spec-validation
  (dt-test-cases/assert-cases-satisfied-by
    (partial spec/valid? :datatype.phone/phone-number-string)
...
(deftest phone-number-string-spec-generation
  (testing "does not generate nil"
    (is (every? false?
          (map nil?
            (gen/sample
              (spec/gen :datatype.phone/phone-number-string) 100)))))
  ...
  (testing "generates unique phone number strings"
    (let [phone-number-strings
          (into #{}
            (gen/sample
              (spec/gen :datatype.phone/phone-number-string)
              100))]
...
(deftest phone-number-string-pred-requirement
  (is (= :must-be-a-phone-number-string
        (sv-core/pred-requirement
          'datatype.phone.core/phone-number-string?))))
