(ns spec.definition.uri.core-test
  (:require
   ...
   [spec.definition.uri.core :as sd-uri]
...
(deftest http-uri-string?-as-requirement
  (is (= :must-be-an-http-uri-string
        (sv-core/pred-requirement
          'spec.definition.uri.core/http-uri-string?))))
...
(deftest http-uri-string?-as-generator
  (testing "does not generate nil"
    (is (every? false?
          (map nil?
            (gen/sample
              (spec/gen sd-uri/http-uri-string?) 100)))))
  ...
  (testing "generates with only http and https schemes"
    (let [schemes
          (into #{}
            (map #(:scheme (uri/uri %))
              (gen/sample (spec/gen sd-uri/http-uri-string?) 100)))]
  ...
  (testing "uses IPv4 addresses for some hosts"
    (let [hosts
          (into #{}
            (map #(:host (uri/uri %))
              (gen/sample (spec/gen sd-uri/http-uri-string?) 1000)))
          ipv4-address-hosts
          (filter
  ...
  (testing "generates paths with 1, 2, 3 and 4 segments"
    (let [paths
          (into #{}
            (map #(:path (uri/uri %))
              (gen/sample (spec/gen sd-uri/http-uri-string?) 10000)))
          paths
