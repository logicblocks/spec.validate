...
(deftest uk-postcode-formatted-string-spec-validation
  (dt-test-cases/assert-cases-satisfied-by
    (partial spec/valid? :datatype.address/uk-postcode-formatted-string)
...
(deftest uk-postcode-formatted-string-spec-generation
  (testing "does not generate nil"
    (is (every? false?
          (map nil?
            (gen/sample
              (spec/gen :datatype.address/uk-postcode-formatted-string) 100)))))
  ...
  (testing "generates unique UK postcodes"
    (let [postcodes
          (into #{}
            (gen/sample
              (spec/gen :datatype.address/uk-postcode-formatted-string)
              100))]
      (is (= (count postcodes) 100)))))

(deftest uk-postcode-formatted-string-pred-requirement
  (is (= :must-be-a-uk-postcode
        (sv-core/pred-requirement
          'datatype.address.core/uk-postcode-formatted-string?))))
